...
| FAD_WU1 | httproxy | WINDOW_UPDATE flow control overflow — guard at 0x2a082c confirms RFC 7540 §6.9.1 safe | ELIMINATED |
| FAD_CHUNKSIZE | httproxy | Chunked transfer hex digit overflow — pre-shift cap at 0x7FFFFFF + ring-buffer min() confirms safe | ELIMINATED |
...
- 2026-09-26 (session 10): httproxy H2/chunked integer overflow surface completed. HPACK integer decoder (0x295850): 3 guards SAFE. HPACK header alloc (0x2a3113): 0x4000 cap SAFE. WINDOW_UPDATE handler (0x2a07d0): RFC 7540 §6.9.1 check at 0x2a082c (0x7fffffff-increment < current_window → FLOW_CONTROL_ERROR) SAFE. Chunked hex accumulator (http2_rsp_body_read_callback, 0x1f2d80): pre-shift cap cmp [rsp+8],0x7ffffff at 0x1f2de4; chunk_size stored [r15+0x34] at 0x1f34e9; body copy bounded by min(remaining,ring_buf) at 0x1f2b28. All SAFE. FAD_WU1+FAD_CHUNKSIZE ELIMINATED. httproxy memory corruption surface analysis COMPLETE.
- 2026-09-26 (session 12): libwaf.so PENDING items resolved. __snprintf_chk×233: 220 from .rodata SAFE; 13 scanner misses — 10 also .rodata (scan window), 2 dead code (0 callers), 1 hex-only output no '%'. waf_blk_ip_get callers (cli/libcgo) pass .rodata fmt SAFE. __vsnprintf_chk×12: waf_system SAFE; waf_chunk_printf*/vprintf_ex dead (0 callers); 3 private static debug fns dead; waf_http_cssp_reg/waf_wje_readjson/cfg_openapi_schema_import_file admin-only PLAUSIBLE LOW. SQLite3: waf_db_*() APIs (select * from %s where %s) have 0 callers in entire firmware — dead code ELIMINATED. PCRE: pcre_compile_exp/match_exp* exported wrappers 0 internal callers, 0 external importers; libwaf.so uses lexertl not pcre internally — ELIMINATED. All 3 libwaf.so PENDING items CLOSED.
- 2026-09-26 (session 11): httproxy remaining memory corruption surface — H2 frame padding validation, H1 body, NTLM, sprintf/strcpy, WebSocket, TLS. DATA pad guard at 0x2a19a9: movzx(pad_length) + cmp [r15+0x268],rax; jb → error SAFE. PUSH_PROMISE pad: two guards at 0x2a219c/0x2a21b4 (pad fits + 4-byte promised_stream_id fits) SAFE. HEADERS pad: two guards at 0x2a2c6d/0x2a2c85 SAFE. CONTINUATION: state classifier stub at 0x2b36d0 reads [rbp+0x57] flag; unexpected path = cold log block at 0x2a1e80 (lea→log_error→return, no memory ops); expected path re-enters verified HPACK decoder SAFE. H1 body: HAProxy ring buffer architecture (tune.bufsize, cmova min() pattern) caps all I/O — no unchecked Content-Length malloc SAFE. NTLM malloc at 0x14c3cd: malloc(base64_decode_return); base64_decode enforces max_out_len=[r13+0x198]-1 before returning size; test eax,eax; js → early return; malloc only if positive and bounded SAFE. sprintf×6: all callers use fixed format literals from .rodata (%.1f, %ld, UUID %02x..., Content-Length) — no user-controlled format strings SAFE. strcpy×53: all in 0x6c-0x7f address range (WAF/scripting/config area), none in HTTP request path (0x14-0x15); sources are register values not constants; HAProxy ring buffer architecture bounds all network I/O before data reaches these — UNVERIFIED LOW. WebSocket: no WS frame parser in httproxy; connection proxied as opaque TCP after Upgrade flag 0x2000000 set at 0x14d9c5 SAFE. TLS SSL_read: like recv, bounded by caller-supplied buffer size SAFE. HTTPROXY MEMORY CORRUPTION SURFACE 100% COMPLETE. Only finding: FAD_H4 CONFIRMED (SNI OOB read).
- 2026-09-26 (session 13): PLAUSIBLE LOW audit pass — all remaining queued items resolved. FAD_M1 ELIMINATED: miglogd fadcsystem 'rm -rf /var/log/logrpt/%s' uses VDOM name validated by is_valid_host_name (libcmdb_plugin.so 0x84f00) → [A-Za-z0-9_-]; no path traversal (no '/' or '..' possible). FAD_BGPD class ELIMINATED: bgpd×236/ospfd×101/ospf6d×65 system() callers all log-rotation pattern ('cp /tmp/%s_bgpd.log ...') with daemon-internal log filenames, never user-supplied values. FAD_S1 ELIMINATED: saml_sp_metadata.sh '"' injection gated by CMDB field validator for SAML entity ID — same is_valid_host_name class; no '"' allowed in entity ID field. lb cmf_exec_conf downgraded from PLAUSIBLE MEDIUM to PLAUSIBLE LOW: is_valid_host_name blocks '"'/CRLF in all name fields (scripting name/VDOM/VS/ADFS-pub-name); scripting-file path also filename-class validated; no known injection path. FAD_AV1 ELIMINATED: avIsIgnoreBuffer actual export = 0xc4180 (not 0xf3531 as prior session misidentified); 0xf3531 strcpy is in internal MIME parser 0xf33d0; field extractor 0xf6320 gate (cmp eax,[rsp+0x40] limit=0x14=20) bounds all writes to <20 bytes before strcpy into 64-byte slot — no overflow. libwaf.so waf_wje_readjson vsnprintf ELIMINATED: format at 0x212fe0 = fixed timestamp '%4d-%02d-%02d %02d:%02d:%02d %010lu\t', no user components. fnginxctld fadcpopen×2 fnginx-t (0x1ea3c/0x2c15b) ELIMINATED: '/bin/fnginx[_new] -t -p %s/%s -c %s/%s/fnginx.conf' — %s = VS name validated → [A-Za-z0-9_-]; no shell metacharacters reachable. flg_accessd fadcpopen 0xe3b8 ELIMINATED: '/bin/rm -rfv /var/log/logrpt/<vdom>/resdir/* 2>&1' — VDOM name validated → [A-Za-z0-9_-]. lb fadcpopen grep 0x753c0 ELIMINATED: single-quote breakout impossible ([A-Za-z0-9_-] contains no quote chars). Remaining PLAUSIBLE LOW: scanvirFileBuffer strcpy 0x93f18 (AV result struct, no callers in firmware); flg_accessd fadcsystem tar (admin log download); vtl system() HSM cmds; libwaf waf_http_cssp_reg + cfg_openapi vsnprintf (admin format args); lb /bin/sh mkstemp write-content untraced; lb cmf_exec_conf CRLF residual.
- 2026-09-26 (session 14): PLAUSIBLE LOW cleanup — remaining items resolved. del_netdev ELIMINATED: __snprintf_chk at 0x1990 → system(); r8='/bin/ls %s > /tmp/tmp.brg.list' AND r9='/sys/class/net/' both RIP-relative .rodata constants; usage { 0 | 1 | 2 } confirms mode arg not bridge name; command hardcoded. lb mkstemp+/bin/sh ELIMINATED: 0x23ad0 writer (7 callers) use VS/VDOM names [A-Za-z0-9_-] → no shell metacharacters in script body; 0x7b110 lb_ha_sync_file callers (0x78c6c/0x78cb7) build scripts with all %s = hardcoded .rodata paths ('/var/log/sync_stick_table', '/tmp/lbtables.tar'). CRITICAL ARCH CORRECTION — fadcsystem redirect mechanism: parse_redirect_path (libstdext.so 0x2560) handles '<','>>','2>','&>' ONLY via byte comparisons (0x3c/0x3e/0x32/0x26); implements as posix_spawn file-actions (dup2+open); shell metacharacters ';','|','$()','`' NOT handled → literal argv. Prior claim '>> is literal argv' was wrong for redirect characters; correct is 'handled as file-action'. Security verdict unchanged (no shell injection). httproxy3 filter_sessions_table_4 ELIMINATED: 'cat %s >> %s' where src='/proc/net/ip_vs_session' (hardcoded .rodata), dest='/var/log/vs/<VS_name>/session_table' (VS name [A-Za-z0-9_-]); via sys_vdom_exec→fadcsystem posix_spawn+file-action. cli more %s ELIMINATED: system('more /var/log/vs/<vsname>/waf_blocked_ip') — vsname validated [A-Za-z0-9_-]. upgrade.sh |sh pattern at line 41 confirmed (in addition to eval at lines 24/37); all require write access to /var/log/logrpt/ (admin/root). Remaining PLAUSIBLE LOW (FINAL): scanvirFileBuffer strcpy 0x93f18 (AV result struct, no callers in firmware); vtl system() HSM cmds (SafeNet Luna, admin-only); libwaf waf_http_cssp_reg + cfg_openapi vsnprintf (admin format args); lb cmf_exec_conf CRLF residual; upgrade.sh eval+sh (admin/root prereq); upgrade-config-from3.x.sh eval (requires write to /migadmin/etc/); keepalived execle /bin/bash (admin VRRP script, by-design); miglogd cp_callers (admin plugin path, posix_spawn); libcmdb_plugin.so file-op path traversal class (admin VDOM/schema/bookmark names, '/' blocked by is_valid_host_name).
