"""Path-insensitive, flow-sensitive dataflow analysis over a PPC32/64 CFG.

Usage:
    from ablation.analyzers.analysis_ppc import analyze_cfg
    ...
    init = TaintState(Mode.PPC64)
    init.taint_reg("r3", "user")
    result = analyze_cfg(list(from_listing(text)), Mode.PPC64, initial=init)
...
from .cfg_ppc import CFG, build_cfg
from .insn_ppc import Insn
from .isa_ppc import Mode
from .taint_tracker_ppc_labeled import Finding, Policy, TaintState, TaintTracker
...
def analyze_cfg(
    ...
    entry_states: Dict[int, TaintState] = {cfg.entry: (initial or TaintState(mode)).copy()}
    exit_states: Dict[int, TaintState] = {}
    visits: Dict[int, int] = {}
