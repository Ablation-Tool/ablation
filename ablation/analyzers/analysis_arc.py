"""Path-insensitive, flow-sensitive dataflow analysis over an ARC EM/HS CFG.

Usage:
    from ablation.analyzers.analysis_arc import analyze_cfg
    from ablation.analyzers.taint_tracker_arc_labeled import TaintState
    from ablation.analyzers.isa_arc import Mode
    from ablation.analyzers.insn_arc import from_listing

    init = TaintState()
    init.regs["r0"] = Taint(frozenset({"user"}), None)
    result = analyze_cfg(list(from_listing(text)), Mode.HS, initial=init)
...
from .cfg_arc import CFG, build_cfg
from .insn_arc import Insn
from .isa_arc import Mode
from .taint_tracker_arc_labeled import Finding, Policy, TaintState, TaintTracker
